...
export function TodaysForecastCard({ data }: WeatherForecastCardProps) {
	...
	return (
		<Card className="w-full">
			<CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
				<CardTitle className="text-2xl text-muted-foreground font-medium">TODAY'S FORECAST</CardTitle>
			</CardHeader>
			<CardContent>
				<div className="flex flex-row justify-evenly items-center">{renderForecastForDate(groupedData[new Date().toISOString().split('T')[0]])}</div>
			</CardContent>
		</Card>
	);
}
